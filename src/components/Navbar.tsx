...
export function Navbar() {
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-3">
            {!isHomePage && (
              <Button 
                variant="ghost" 
                size="icon" 
                className="mr-1 rounded-full hover:bg-white/5" 
                onClick={handleBack}
              >
            ...
            <Link href="/" className="flex items-center gap-2 group">
              ...
              <span className="font-headline text-lg font-bold tracking-tight text-[#F8FAFC]">
                QUANTUM NEST
              </span>
            </Link>
          </div>
             ...
             <Sheet>
               <SheetTrigger asChild>
                 <Button 
                   variant="ghost" 
                   size="icon" 
                   className="rounded-full bg-white/5 hover:bg-white/10 transition-all border border-white/10"
                 >
                   <Menu className="w-5 h-5 text-white" />
                 </Button>
               </SheetTrigger>
               <SheetContent side="right" className="bg-[#050816] border-l border-white/10 w-[300px] p-0">
                 <ScrollArea className="h-full w-full p-6">
                   <SheetHeader className="text-left mb-8">
                     <SheetTitle className="font-headline text-2xl font-bold text-[#F8FAFC]">Explorer Hub</SheetTitle>
                     <SheetDescription className="text-[#94A3B8]">Recalibrate your mission path</SheetDescription>
                   </SheetHeader>

                   <div className="space-y-6">
                     <div className="glass-card p-5 rounded-2xl border-white/10 shadow-[0_0_20px_rgba(93,169,255,0.2)]">
                        {user ? (
                          <div className="space-y-4">
                            <Link href="/profile" className="flex items-center gap-4 group">
                              ...
                              <div className="overflow-hidden">
                                <p className="font-bold text-[#F8FAFC] truncate group-hover:text-primary transition-colors">
                                  {profile?.displayName || user.displayName || 'Unnamed Explorer'}
                                </p>
                                <p className="text-[10px] text-[#94A3B8] truncate uppercase tracking-widest">
                            ...
                            <div className="grid grid-cols-1 gap-2">
                               <Button variant="ghost" size="sm" className="w-full justify-start text-primary hover:bg-white/5 gap-3 h-11" asChild>
                                  <Link href="/profile">
                          ...
                          <div className="text-center py-2">
                            <Button asChild className="w-full font-bold rounded-xl h-12 text-[10px] uppercase tracking-widest">
                     ...
                     <div className="space-y-1">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-[#94A3B8] mb-3 px-2">Archives</p>
                        {[
                          ...
                          <Button 
                            key={item.href} 
                            variant="ghost" 
                            className="w-full justify-start gap-4 rounded-xl py-6 hover:bg-white/5 transition-colors" 
                            asChild
                          >
                            <Link href={item.href} onClick={(e) => handleNavAccess(e, item.href)}>
                              <item.icon className="w-5 h-5 text-primary" /> <span className="font-headline text-sm font-medium text-[#CBD5E1]">{item.name}</span>
                            </Link>
                          </Button>
                        ))}
                     </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-[#050816]/90 backdrop-blur-xl border-t border-white/10 pb-safe md:hidden">
                ...
                <div key={item.href} className="flex justify-center -mt-6">
                  <Link
                    ...
                    className={cn(
                      ...
                      isActive ? "cosmic-gradient text-white shadow-primary/40" : "bg-[#0B1023] border border-white/10 text-primary shadow-[0_0_15px_rgba(93,169,255,0.3)]"
                    )}
                  >
